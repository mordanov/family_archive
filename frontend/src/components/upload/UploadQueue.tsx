                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-ink-muted">
                      {t('upload.status', { status: t(`upload.statuses.${u.status}`) })} • {t('upload.bytesUploaded', { uploaded: formatBytes(u.bytesUploaded), total: formatBytes(u.size) })}
                      {u.status === 'paused' && !u.file && ' • ' + t('upload.repickFile')}
                      {u.error && <span className="text-red-600"> • {u.error}</span>}
                    </div>
                  </div>
                  <div className="flex items-center gap-1">
                    {u.status === 'uploading' && (
                      <button onClick={() => pause(u.localId)} className="rounded p-1 hover:bg-surface-muted" title={t('upload.pause')}>
                        <Pause size={14} />
                      </button>
                    )}
                    {(u.status === 'paused' || u.status === 'error') && u.file && (
                      <button onClick={() => resume(u.localId)} className="rounded p-1 hover:bg-surface-muted" title={t('upload.resume')}>
                        <Play size={14} />
                      </button>
                    )}

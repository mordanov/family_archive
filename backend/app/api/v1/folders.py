...
async def create_folder(payload: FolderCreate, request: Request, user: CurrentUser, db: AsyncSession = Depends(get_db)):
    ...
    f = await folders_repo.create(db, payload.parent_id, name, user.sub  # TODO(data-migration): sub is UUID; DB audit/folders expect integer user_id)
    await audit_repo.log(db, user_id=user.sub  # TODO(data-migration): sub is UUID; DB audit/folders expect integer user_id, action="mkdir", entity_type="folder", entity_id=f.id, ip=_ip(request))
    return f
...
async def patch_folder(folder_id: int, payload: FolderPatch, request: Request, user: CurrentUser, db: AsyncSession = Depends(get_db)):
    ...
    if payload.name is not None:
        ...
        await audit_repo.log(db, user_id=user.sub  # TODO(data-migration): sub is UUID; DB audit/folders expect integer user_id, action="rename", entity_type="folder", entity_id=f.id, ip=_ip(request))
    if payload.parent_id is not None and payload.parent_id != f.parent_id:
        await folders_repo.move(db, f, payload.parent_id)
        await audit_repo.log(db, user_id=user.sub  # TODO(data-migration): sub is UUID; DB audit/folders expect integer user_id, action="move", entity_type="folder", entity_id=f.id, ip=_ip(request))
    return f
...
async def delete_folder(folder_id: int, request: Request, user: CurrentUser, db: AsyncSession = Depends(get_db)):
    ...
    await audit_repo.log(db, user_id=user.sub  # TODO(data-migration): sub is UUID; DB audit/folders expect integer user_id, action="delete", entity_type="folder", entity_id=folder_id, ip=_ip(request))
    return None

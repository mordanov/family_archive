...
async def restore_file(file_id: int, request: Request, user: CurrentUser, db: AsyncSession = Depends(get_db)):
    # TODO(data-migration): user.sub is a UUID string; trash service expects integer user_id
    return await trash_service.restore_file(db, file_id, None, _ip(request))
...
async def restore_folder(folder_id: int, request: Request, user: CurrentUser, db: AsyncSession = Depends(get_db)):
    # TODO(data-migration): user.sub is a UUID string; trash service expects integer user_id
    return await trash_service.restore_folder(db, folder_id, None, _ip(request))
